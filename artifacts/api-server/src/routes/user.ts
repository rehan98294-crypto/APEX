...
// Security guard: new balance may not exceed the current DB balance by more than $1,000
// to prevent client-side inflation attacks.
router.post("/user/balance-sync", requireAuth, async (req, res) => {
  ...
  try {
    // Fetch current DB balance to enforce server-side cap
    ...
    const currentBalance = parseFloat(String(currentUser.balance ?? 0));
    const MAX_SINGLE_SYNC_INCREASE = 1000;

    if (balance > currentBalance + MAX_SINGLE_SYNC_INCREASE) {
      console.warn(`[UserBalance] Suspicious sync rejected — user=${userId} current=${currentBalance} requested=${balance}`);
      return res.status(400).json({ error: "Balance sync exceeds allowed limit." });
    }
...
// POST /api/user/save-withdraw-address — upsert withdrawal addresses (authenticated)
router.post("/user/save-withdraw-address", requireAuth, async (req, res) => {
